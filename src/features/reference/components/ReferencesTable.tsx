...
export default function ReferencesTable({ references, loading, groups }: Props) {
  const { setReferenceToEdit, openEditReferenceDialog, setTitleReferenceDialog, setIsEdit } = useReferencesContext();
  ...
  const handleClickEditReferences = (dataTable: MUIDataTableMeta<unknown>) => {
    const groupToEdit = groups.find((group) => group.id === dataTable.rowData[3]);
  ...
  const columns: MUIDataTableColumnDef[] = [
    ...
    { name: 'referenceDescription', label: 'Descripción' },
    {
      ...
      options: {
        customBodyRender: (_, dataTable) => {
          return <Typography>{groups.find((group) => group.id === dataTable.rowData[3])?.name}</Typography>;
        },
      },
    },
