...
export default function FaultCausalsEditDialog({ getFaultCausals }: Props) {
    ...
    try {
      ...
      if (res.data) {
        ...
        toastsManager.showToast(
          "success",
          `Fallas y causales ${text} correctamente`
        );
  ...
  return (
    <Dialog
      open={openEditFaultCausalDialogState}
      onClose={closeEditFaultCausalDialog}
      fullWidth
      maxWidth="md"
    >
      <form noValidate onSubmit={handleSubmit(handleOnSave)}>
        ...
        <DialogActions>
          ...
          <Button
            variant="contained"
            color="inherit"
            onClick={closeEditFaultCausalDialog}
          >
            Cancelar
          </Button>
        </DialogActions>
