...
import { Close, DoDisturb, LockOpen, MoreVert, RemoveRedEye } from '@mui/icons-material';
...
import { ActionEnum, IServiceOrders, ServicesOrderStateEnum } from '../models/ServiceOrders.type';
...
export default function ServiceOrdersTableMenu({ seviceOrder }: props) {
  ...
  const { openCloseServiceOrderDialog, setTitleServiceOrderDialog, setActionTodo } = useServiceOrdersContext();
  ...
  return (
    <div>
      ...
      >
        ...
        {(role === RolesEnum.TECNICO || role === RolesEnum.AUXILIAR || RolesEnum.ADMINSTRADOR) && (
        ...
        {role === RolesEnum.ADMINSTRADOR && (
          <MenuItem onClick={() => handleClose(ActionEnum.ANULAR)} disableRipple>
            <DoDisturb />
            Anular
          </MenuItem>
        )}
        {(role === RolesEnum.TECNICO || role === RolesEnum.AUXILIAR || RolesEnum.ADMINSTRADOR) && (
          <MenuItem onClick={() => handleClose(ActionEnum.CERRAR)} disableRipple>
            <Close />
            Cerrar
          </MenuItem>
        )}
        {RolesEnum.ADMINSTRADOR && seviceOrder?.status === ServicesOrderStateEnum.BLOQUEADAS && (
          <MenuItem onClick={() => handleClose(ActionEnum.DESBLOQUEAR)} disableRipple>
            <LockOpen />
            Desbloquear
          </MenuItem>
        )}
