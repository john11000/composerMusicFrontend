...
import { IServiceOrdersContext, IServiceOrders, ActionEnum } from '../models/ServiceOrders.type';
import { PropsProvider } from '@/models/context.type';

export const ServiceOrdersContext = createContext<IServiceOrdersContext | undefined>(undefined);

export const ServiceOrdersProvider = ({ children }: PropsProvider) => {
  const [serviceOrderToEdit, setServiceOrderToEdit] = useState<IServiceOrders | undefined>();
  const [openEditServiceOrderDialogState, setopenEditServiceOrderDialogState] = useState(false);
  const [openCloseServiceOrderDialogState, setopenCloseServiceOrderDialogState] = useState(false);
  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [titleServiceOrderDialog, setTitleServiceOrderDialog] = useState<string>('');
  const [action, setActionTodo] = useState<ActionEnum>(ActionEnum.ASIGNAR_TECNICO);
...
export const useServiceOrdersContext = (): IServiceOrdersContext => {
  ...
  if (context === undefined) {
    throw new Error('ServiceOrdersContext debe usarse dentro de ServiceOrdersProvider');
  }
