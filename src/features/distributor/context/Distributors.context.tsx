...
import { IDistributorsContext, IDistributors } from '../models/Distributors.type';
import { PropsProvider } from '@/models/context.type';

export const DistributorsContext = createContext<IDistributorsContext | undefined>(undefined);

export const DistributorsProvider = ({ children }: PropsProvider) => {
  const [distributorToEdit, setDistributorToEdit] = useState<IDistributors | undefined>();
  const [openEditDistributorDialogState, setopenEditDistributorDialogState] = useState(false);
  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [titleDistributorDialog, setTitleDistributorDialog] = useState<string>('Crear Distribuidore');
...
export const useDistributorsContext = (): IDistributorsContext => {
  ...
  if (context === undefined) {
    throw new Error('DistributorsContext debe usarse dentro de DistributorsProvider');
  }
