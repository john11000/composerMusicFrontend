...
export default function CustomersTable({ customers, loading, departaments }: Props) {
  const { setCustomerToEdit, openEditCustomerDialog, setTitleCustomerDialog, setIsEdit, setIsEditCustomer } =
    useCustomersContext();
  const { openEditInvoiceDialog, setTitleInvoiceDialog, setIsEdit: setIsEditinvoice } = useInvoicesContext();
  const { updateCustomer, loading: loadingUpdateCustomer } = useUpdateCustomer();
  ...
  const handleEditStateCustomer = async (e: ChangeEvent<HTMLInputElement>, dataTable: MUIDataTableMeta<unknown>) => {
    ...
    },
    {
      ...
      options: {
        customBodyRender(firstName, dataTable) {
          return (
            <th key={firstName.index} className={firstName.classes} style={{ textAlign: 'center' }}>
      ...
      options: {
        ...
        customBodyRender: (_, dataTable) => {
          if (role === RolesEnum.ADMINSTRADOR) {
            return (
              <Typography>
                <Switch checked={dataTable.rowData[6]} onChange={(e) => handleEditStateCustomer(e, dataTable)} />
