...
export const CustomersProvider = ({ children }: PropsProvider) => {
  const [customerToEdit, setCustomerToEdit] = useState<Partial<ICustomers> | undefined>();
  const [openEditCustomerDialogState, setopenEditCustomerDialogState] = useState(false);
  const [isEdit, setIsEdit] = useState<boolean>(false);
  const [titleCustomerDialog, setTitleCustomerDialog] = useState<string>('Crear grupo');
