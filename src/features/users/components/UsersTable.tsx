...
export default function UsersTable({ users, loading }: Props) {
  const { setUserToEdit, openEditUserDialog, setTitleUserDialog, setIsEdit } = useUsersContext();
  ...
  const onChangeUserState = async (e: ChangeEvent<HTMLInputElement>, dataTable: MUIDataTableMeta<unknown>) => {
      ...
      options: {
        customBodyRender: (_, dataTable) => {
          if (role === RolesEnum.ADMINSTRADOR) {
            ...
            return <Typography variant="body2">{`${dataTable.rowData[1]} ${dataTable.rowData[2]}`}</Typography>;
          }
        },
      },
    },
      ...
      options: {
        customBodyRender: (_, dataTable) => {
          if (role === RolesEnum.ADMINSTRADOR) {
            return (
              <Typography>
                <Switch checked={dataTable.rowData[6]} onChange={(e) => onChangeUserState(e, dataTable)} />
