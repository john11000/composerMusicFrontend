...
const formatAddress = (data: InvoiceItems) => {
  const { typeLocation, locationNumber, numberLocation, secondNumberLocation } = data;
...
const desestructuAddress = (address: string) => {
  ...
  address = address.replace(/# /g, '');
  address = address.replace(/- /g, '');
  const [typeLocation, locationNumber, numberLocation, secondNumberLocation] = address.split(' ');
