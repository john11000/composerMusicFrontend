...
export const CalculateWarrantyDays = (warrantyPeriod: string, warrantyDaysRefernce: number): number => {
  const daysWarranty = differenceInDays(parse(warrantyPeriod, 'yyyy-MM-dd', new Date()), today);
